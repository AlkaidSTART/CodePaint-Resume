      ...
      <div className="mx-auto max-w-7xl">
        <div className="relative flex min-h-16 items-center gap-4 overflow-hidden rounded-[14px] border border-black/15 bg-black/[0.035] px-4 shadow-[0_10px_35px_rgba(20,20,20,0.1)] backdrop-blur-2xl backdrop-saturate-150 sm:min-h-[72px] sm:px-6">
          ...
          <div className="header-item ml-auto flex shrink-0 items-center gap-1 sm:gap-2">
            <a className="header-item group flex min-h-11 min-w-0 items-center gap-2 rounded-md px-2 py-2 text-sm text-black/70 outline-offset-4 transition-colors hover:text-sky-600 focus-visible:outline-2 focus-visible:outline-black sm:px-3" href={albumUrl} target="_blank" rel="noreferrer">
            ...
            <button className="header-item group flex min-h-11 shrink-0 items-center gap-2 rounded-md px-2 py-2 text-sm text-black/70 outline-offset-4 transition-colors hover:text-sky-600 active:text-sky-700 focus-visible:outline-2 focus-visible:outline-black disabled:cursor-wait disabled:opacity-60 sm:px-3" type="button" onClick={onApply}>
              <span>前往投递</span>
              <span className="inline-block text-base transition-transform group-hover:translate-x-1" aria-hidden="true">→</span>
            </button>
          </div>
        </div>
      </div>
