...
export function ApplyModal({ isOpen, onClose, defaultRole = "dev" }: ApplyModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl border border-slate-200/80 sm:p-8 max-h-[92vh] overflow-y-auto">
        <button
          onClick={onClose}
        ...
        {submitted ? (
          <div className="py-8 text-center">
            ...
            <h3 className="mt-4 text-lg font-bold text-slate-900">报名材料及简历已成功提交</h3>
            <p className="mt-2 text-xs leading-relaxed text-slate-600">
              简历文件 <strong className="text-slate-800 font-medium">[{pdfFile?.name}]</strong> 已成功解析并归档。<br />
            ...
            <div className="space-y-3.5 pt-1">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-start rounded-xl border p-3 text-left transition ${
                        role === r.id
                          ? "border-sky-500 bg-sky-50/70 text-sky-900 font-semibold shadow-2xs"
                          : "border-slate-200 bg-slate-50/50 text-slate-600 hover:border-slate-300 hover:bg-slate-50"
                      }`}
                    >
                      <span className="text-xs font-bold">{r.label}</span>
                      <span className="mt-0.5 text-[10px] text-slate-500">{r.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="你的名字"
                    className="w-full rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:border-sky-500 focus:bg-white focus:outline-hidden"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[10px] text-slate-400">仅支持 .pdf 格式，20MB 以内</span>
                </div>
                ...
                {!pdfFile ? (
                  ...
                  >
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-sky-50 text-sky-600 transition group-hover:scale-105">
                  ...
                  <div className="flex items-center justify-between rounded-xl border border-sky-200 bg-sky-50/60 p-3.5">
                    ...
                    <button
                      ...
                      className="ml-2 rounded-lg p-1.5 text-slate-400 hover:bg-white hover:text-rose-600 transition shrink-0"
                      title="移除重新选择"
                    >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:border-sky-500 focus:bg-white focus:outline-hidden"
                />
              </div>
            </div>
