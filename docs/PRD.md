...
### 3.2 AI 结果必须可解释与准则化 (Ashby 范式)

页面严禁只有大模型随机构建的“主观感觉分”（如单纯的 `匹配度 89%`）。

AI 必须遵循岗位设定的硬性与软性准则（Criteria），输出结构化证据：

- 准则判断（`met` / `partial` / `not_met` / `unknown`）；
- 精确到页码与原句的佐证（`Evidence: page + quote`），前端支持点击直接高亮定位原简历；
- 后端确定性加权求和算法生成最终可解释得分；
- 审核员可针对单个准则进行人工覆写（Human Override），系统自动统计 AI 与人类审核员的不一致率（Disagreement Rate）；
- AI 不做任何自动录取/淘汰裁决，最终决策权完全归属招新团队成员。
...
## 19. 插件体系与外部生态联动 (Plugin Architecture)

### 19.1 业务诉求
候选人在管理后台完成招新流转（如由初筛/面试推进至 `passed` 录用状态）后，HR 不需要手动复制候选人信息到聊天工具建群，系统通过可插拔的插件体系自动联动外部办公软件。

### 19.2 飞书协同插件 (Feishu Plugin)
- **触发时机**：候选人状态流转为 `passed`。
- **执行逻辑**：
  1. 异步事件消费者捕获 `application.status.changed` 事件；
  2. 提取候选人姓名、联系方式、拟录用岗位及面试评价亮点；
  3. 组装为飞书富文本交互式卡片（Interactive Message Card）；
  4. 投递至对应方向的飞书招新答疑群或大群 Webhook；
  5. 记录投递日志、响应状态与重试记录至 `plugin_events` 表。
- **扩展性**：支持后续平滑挂载钉钉、企业微信机器人或自动化发送邮件通知驱动。
